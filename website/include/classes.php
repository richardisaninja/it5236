...
*/
    public function auditlog($context, $message, $priority = 0, $userid = NULL) {
      ...
      $url = "https://txy5ujggpe.execute-api.us-east-1.amazonaws.com/default/auditLog";
      $data = array(
        'context' => $context,
        'message'=>$message,
        'ipaddress'=>$ipaddress,
      ...
      curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json','Content-Length: ' . strlen($data_json), 'x-api-key: cqNZl0iunH1ODjVeSQtKc7fch2cYmrusap63O8MY'));
    ...
    public function register($username, $password, $email, $registrationcode, &$errors) {
        ...
        if (sizeof($errors) == 0) {
			...
			$url = "https://txy5ujggpe.execute-api.us-east-1.amazonaws.com/default/registerUser";
			$data = array(
				'userid'=>$userid,
				'username'=>$username,
			...
			$ch = curl_init();
			curl_setopt($ch, CURLOPT_URL, $url);
			curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json','Content-Length: ' . strlen($data_json), 'x-api-key: KMHDdfyIT98Ax10rdNO2X9dcZNtElgS8xIpm6yK1'));
			curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'POST');
			curl_setopt($ch, CURLOPT_POSTFIELDS, $data_json);
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
			$response  = curl_exec($ch);
    ...
    */
    ...
    protected function sendValidationEmail($userid, $email, &$errors) {
      ...
      $url = "https://txy5ujggpe.execute-api.us-east-1.amazonaws.com/default/sendValidationEmail";
      $data = array(
      ...
      $ch = curl_init();
      curl_setopt($ch, CURLOPT_URL, $url);
      curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json','Content-Length: ' . strlen($data_json), 'x-api-key: 7nWfz4OuUP4XvrPVGocyJaJQLSa6c4Cl2GmZPRSo'));
      curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'POST');
      curl_setopt($ch, CURLOPT_POSTFIELDS, $data_json);
      curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
      $response  = curl_exec($ch);
...
*/
    ...
    public function newSession($userid, &$errors, $registrationcode = NULL) {
      ...
      $url = "https://txy5ujggpe.execute-api.us-east-1.amazonaws.com/default/newSession";
      $data = array(
        'sessionid'=>$sessionid,
        'userid'=>$userid,
        'passwordHash'=>$passwordhash,
      ...
      $ch = curl_init();
      curl_setopt($ch, CURLOPT_URL, $url);
      curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json','Content-Length: ' . strlen($data_json), 'x-api-key: omUx0h8sV575SRVIFSDFS6cZRnwV63cW9ThmhstG'));
      curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'POST');
      curl_setopt($ch, CURLOPT_POSTFIELDS, $data_json);
      curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
      $response  = curl_exec($ch);
...
public function getUserRegistrations($userid, &$errors) {
   ...
   $url = "https://txy5ujggpe.execute-api.us-east-1.amazonaws.com/default/getUseregistration?userid=" . $userid;
   $ch = curl_init();
   curl_setopt($ch, CURLOPT_URL, $url);
       curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json','Content-Length: ' . strlen($data_json), 'x-api-key: KMHDdfyIT98Ax10rdNO2X9dcZNtElgS8xIpm6yK1'));
...
*/
    ...
    public function updateUserPassword($userid, $password, &$errors) {
      ...
      $url = "https://txy5ujggpe.execute-api.us-east-1.amazonaws.com/default/updateUserPassword";
      $data = array(
        'userid'=>$userid,
        'passwordHash'=>$passwordhash
      );
      ...
      $ch = curl_init();
      curl_setopt($ch, CURLOPT_URL, $url);
      curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json','Content-Length: ' . strlen($data_json), 'x-api-key: 9xBtx5WFkC7alILUpATIL25rSikXKjxs4pHbWgTK'));
...
*/
    // CURL for clearPasswordResetRecords
    protected function clearPasswordResetRecords($passwordresetid) {


      $url = "https://txy5ujggpe.execute-api.us-east-1.amazonaws.com/default/clearPasswordResetRecords";
      $data = array(
      ...
      $ch = curl_init();
      curl_setopt($ch, CURLOPT_URL, $url);
      curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json','Content-Length: ' . strlen($data_json), 'x-api-key: 7tzWuf4h7l37fiHgGtl5a4YtNvi1aLGa6GJOe17p'));
...
*/
   ...
   $url = "https://txy5ujggpe.execute-api.us-east-1.amazonaws.com/default/getSessionUser?sessionid=" . $sessionid;

   $ch = curl_init();
   curl_setopt($ch, CURLOPT_URL, $url);
       curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json','Content-Length: ' . strlen($data_json), 'x-api-key: 1l2ZZGEaeF7dhCZOrcYp24Yt9RYGHplu4Z0WlJH4'));
   curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
   curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
   $response  = curl_exec($ch);
   $httpCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
   ...
   if ($response === FALSE) {
     ...
     if($httpCode == 400) {
       ...
       $user = array(
          ...
          'registrationcode'=>json_decode($response)->registrationcode,
          'username'=>json_decode($response)->username,
          'usersessionid'=>json_decode($response)->usersessionid,
...
*/
    // Registers a new user
        ...
        if (sizeof($errors) == 0) {
            ...
            // Create a new user ID
            $userid = bin2hex(random_bytes(16));
      $url = "https://txy5ujggpe.execute-api.us-east-1.amazonaws.com/default/isAdmin";
      $data = array(
        'userid'=>$userid
      );
      $data_json = json_encode($data);
    ...
    public function login($username, $password, &$errors) {
        ...
        if (sizeof($errors) == 0) {
            ...
            // Construct a SQL statement to perform the insert operation
            $sql = "SELECT userid, username, passwordhash, emailvalidated, email FROM users " .
            ...
            if ($result === FALSE) {
                ...
                if (!password_verify($password, $row['passwordhash'])) {
                  ...
                  $sql = "INSERT INTO validatelogin (username, login_hex, email, time) " .
                      "VALUES (:username, :login_hex, :email, NOW())";

                      $stmt = $dbh->prepare($sql);
                      $stmt->bindParam(":email", $email);
                      $stmt->bindParam(":username", $username);
                      $stmt->bindParam(":login_hex", $hexLogin);
                      $result = $stmt->execute();
...
/*
    // Send an email to validate the login
    public function processLoginValidation($username, $validation_key ,$validationid, &$errors) {

      $username = $_GET['username'];
      $cookie_name = "user";
      $cookie_value = $_GET['username'];
      $username = $_COOKIE[$cookie_name];
      setcookie($cookie_name, $cookie_value, time() + (60 * 1), "/"); // 86400 = 1 day
      echo "Value is: " . $username;
        ...
        // Construct a SQL statement to perform the insert operation
        $sql = "SELECT login_hex FROM validatelogin WHERE username = :username ORDER BY time DESC";
        // Run the SQL select and capture the result code
        $stmt = $dbh->prepare($sql);
        $stmt->bindParam(":username", $username);
        $result = $stmt->execute();
        ...
        $hex_login = $row['login_hex'];

        echo "validation key $validation_key";
        echo " hex login $hex_login";
        ...
        if ($result === FALSE) {
          ...
          //if statement worked
          if($hex_login === $validation_key){
              header('Location: list.php');
          }
          else{
            echo "validation key invalid";
          }
            }
...
*/
  ...
  $url = "https://txy5ujggpe.execute-api.us-east-1.amazonaws.com/default/logout";
  $data = array(
  ...
  $ch = curl_init();
  curl_setopt($ch, CURLOPT_URL, $url);
  curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json','Content-Length: ' . strlen($data_json), 'x-api-key: HcsmKqTlt73kY936s6kCO5XSUjvRTu2o8rpIenSX'));
  curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'DELETE');
  curl_setopt($ch, CURLOPT_POSTFIELDS, $data_json);
  curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
  $response  = curl_exec($ch);
...
/*
    ...
    public function getThings(&$errors) {
        ...
        // Construct a SQL statement to perform the select operation
        $sql = "SELECT thingid, filename, thingattachmentid, thingname, convert_tz(things.thingcreated,@@session.time_zone,'America/New_York') as thingcreated, thinguserid, thingattachmentid, thingregistrationcode FROM things LEFT JOIN attachments ON things.thingattachmentid = attachments.attachmentid LEFT JOIN users ON things.thinguserid = users.userid
         WHERE thingregistrationcode = :registrationcode ORDER BY things.thingcreated ASC";
...
*/
    ...
    public function getThings(&$errors) {
       ...
       $url = "https://txy5ujggpe.execute-api.us-east-1.amazonaws.com/default/getThings?thingregistrationcode=" . $registrationcode;
       $ch = curl_init();
       curl_setopt($ch, CURLOPT_URL, $url);
           curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json','Content-Length: ' . strlen($data_json), 'x-api-key: dUSlh2QjJK8mGYh75PffO1v0hVooYkx4a0VbUr1J'));
       curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
       curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
       $response  = curl_exec($ch);
       $httpCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
...
*/
    ...
    public function getThing($thingid, &$errors) {
      ...
      if (sizeof($errors) == 0){
       ...
       $url = "https://txy5ujggpe.execute-api.us-east-1.amazonaws.com/default/getThing?thingid=" . $thingid;
       $ch = curl_init();
       curl_setopt($ch, CURLOPT_URL, $url);
           curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json','Content-Length: ' . strlen($data_json), 'x-api-key: h1vdQlJ16P7OStfJWHz6978S1TIWTwAP9pWrk5AT'));
       curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
       curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
       $response  = curl_exec($ch);
       $httpCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
       ...
       if ($response === FALSE) {
         ...
         if($httpCode == 400) {
           ...
           $thing = array(
             ...
             'thingcreated'=>$get->thingcreated,
             'thinguserid'=>$get->thinguserid,
             'thingattachmentid'=>$get->thingattachmentid,
             'filename'=>$get->filename,
             'thingregistrationcode'=>$get->thingregistrationcode
...
*/
    ...
    public function getComments($thingid, &$errors) {
      ...
      if (empty($thingid)) {
       ...
       $url = "https://txy5ujggpe.execute-api.us-east-1.amazonaws.com/default/getComments?thingid=" . $thingid;
       $ch = curl_init();
       curl_setopt($ch, CURLOPT_URL, $url);
           curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json','Content-Length: ' . strlen($data_json), 'x-api-key: d5eE3Z9MlQ5Ed78pOhLLarXGSUtwnin65ekp3T0h'));
       curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
       curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
       $response  = curl_exec($ch);
       $httpCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
       ...
       if ($response === FALSE) {
         ...
         if($httpCode == 400) {
           ...
           foreach($get as $key){
                $key = array(
                  'commentthingid'=>$get->commentthingid,
                  'commentid'=>$key->commentid,
                  'commenttext'=>$key->commenttext,
                  'commentposted'=>$key->commentposted,
                  'attachmentid'=>$attachmentid,
                  'username'=>$key->username,
                  'filename'=>$key->thinename
                );
                $comments[] = $key;
...
/*
    ...
    if(isset($attachment) && isset(attachment['name'])){
  ...
  $url = "https://txy5ujggpe.execute-api.us-east-1.amazonaws.com/default/saveAttachment";
  $data = array(
    'filename'=>$filename,
    'attachmentid'=>$attachmentid
  );
  ...
  $ch = curl_init();
  curl_setopt($ch, CURLOPT_URL, $url);
  curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json','Content-Length: ' . strlen($data_json), 'x-api-key: q76bhig4f06jHwUSKMyVx2pVnJ0eKf832kvMYk8W'));
  curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'POST');
  curl_setopt($ch, CURLOPT_POSTFIELDS, $data_json);
  curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
  $response  = curl_exec($ch);
...
/*
    ...
    public function addThing($name, $attachment, &$errors) {
      ...
      $url = "https://txy5ujggpe.execute-api.us-east-1.amazonaws.com/default/addThing";
      $data = array(
        'thingid'=>$thingid,
        'name'=>$name,
        'attachmentid'=>$attachmentid,
      ...
      $ch = curl_init();
      curl_setopt($ch, CURLOPT_URL, $url);
      curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json','Content-Length: ' . strlen($data_json), 'x-api-key: GUcLqzhl9O4zEeGLvmwVw6Z6aTfuQIZwasN5DcsJ'));
      curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'POST');
      curl_setopt($ch, CURLOPT_POSTFIELDS, $data_json);
      curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
      $response  = curl_exec($ch);
...
*/
    ...
    // Registers a new user
      ...
      $url = "https://txy5ujggpe.execute-api.us-east-1.amazonaws.com/default/addComment";
      $data = array(
        'commentid'=>$commentid,
        'text'=>$text,
        'userid'=>$userid,
      ...
      $ch = curl_init();
      curl_setopt($ch, CURLOPT_URL, $url);
      curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json','Content-Length: ' . strlen($data_json), 'x-api-key: 6Ly9bPd3sx1gpj6iaTmx71srrwg8FLtF25N9ZS22'));
      curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'POST');
      curl_setopt($ch, CURLOPT_POSTFIELDS, $data_json);
      curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
      $response  = curl_exec($ch);
...
/*
    ...
    public function getUsers(&$errors) {
      ...
      $users = array();
       ...
       $url = "https://txy5ujggpe.execute-api.us-east-1.amazonaws.com/default/getUsers";
       $ch = curl_init();
       curl_setopt($ch, CURLOPT_URL, $url);
           curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json','Content-Length: ' . strlen($data_json), 'x-api-key: d5eE3Z9MlQ5Ed78pOhLLarXGSUtwnin65ekp3T0h'));
       curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
...
/*
    ...
    public function getUser($userid, &$errors) {
      ...
      if (sizeof($errors) == 0){
       ...
       $url = "https://txy5ujggpe.execute-api.us-east-1.amazonaws.com/default/getUser?userid=" . $userid;
       $ch = curl_init();
       curl_setopt($ch, CURLOPT_URL, $url);
           curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json','Content-Length: ' . strlen($data_json), 'x-api-key: cGXcWiCv8C4jhXZCcYLVH9JL2WqIy4Tx8EE1IbRU'));
       curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
       curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
       $response  = curl_exec($ch);
       $httpCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
...
*/
    ...
    public function updateUser($userid, $username, $email, $password, $isadminDB, &$errors) {
      ...
      $url = "https://txy5ujggpe.execute-api.us-east-1.amazonaws.com/default/updateUser";
      $data = array(
        'userid'=>$userid,
        'passwordHash'=>$passwordhash
      );
      ...
      $ch = curl_init();
      curl_setopt($ch, CURLOPT_URL, $url);
      curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json','Content-Length: ' . strlen($data_json), 'x-api-key: QNlOe6CP5CcDv5x7eMDhgwCZyyppQFayoepiMx40'));
      curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'POST');
      curl_setopt($ch, CURLOPT_POSTFIELDS, $data_json);
      curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
      $response  = curl_exec($ch);
...
*/
    ...
    public function updatePassword($password, $passwordresetid, &$errors) {
      ...
      $url = "https://txy5ujggpe.execute-api.us-east-1.amazonaws.com/default/updatePassword";
      $data = array(
        'passwordresetid'=>$passwordresetid
      );
      $data_json = json_encode($data);
      ...
      if ($response === FALSE) {
        ...
        if($httpCode == 400) {
          ...
          $row = json_decode($response)->userid;
          $userid = $row['userid'];
          $this->updateUserPassword($userid, $password, $errors);
          $this->clearPasswordResetRecords($passwordresetid);
        }
...
public function getAttachmentTypes(&$errors) {
   ...
   $url = "https://txy5ujggpe.execute-api.us-east-1.amazonaws.com/default/getAttachmentTypes";
   $ch = curl_init();
   curl_setopt($ch, CURLOPT_URL, $url);
       curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json','Content-Length: ' . strlen($data_json), 'x-api-key: LghT0F2Ddv419jx3ggPJZkj5Aig91L179OSNBEoj'));
   curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
   curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
   $response  = curl_exec($ch);
   $httpCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
...
*/
    ...
    public function newAttachmentType($name, $extension, &$errors) {
      ...
      $url = "https://txy5ujggpe.execute-api.us-east-1.amazonaws.com/default/newAttachmentType";
      $data = array(
      ...
      curl_close($ch);
            ...
            $this->auditlog("adding new attachmenttype Error", $errors);
        }
...
/*
    function deleteThing($thingid ,$errors){
        $dbh = $this->getConnection();

    if (isset($_POST["delete"])){

      $sql = "DELETE FROM things WHERE thingid = :thingid";
...
*/
    // Registers a new user
    public function deleteThing($thingid, &$errors) {

      if(isset($_POST["delete"])){
        // Only try to insert the data into the database if there are no validation errors
        if (sizeof($errors) == 0) {

			$url = "https://txy5ujggpe.execute-api.us-east-1.amazonaws.com/default/deleteThing";
			$data = array(
				'thingid'=>$thingid
			);
			$data_json = json_encode($data);
			$ch = curl_init();
			curl_setopt($ch, CURLOPT_URL, $url);
			curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json','Content-Length: ' . strlen($data_json), 'x-api-key: oZSnIxxm4B8Cd9lNq7Xgf1OwLBcgNNvoQ6GFktph'));
			curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'DELETE');
			curl_setopt($ch, CURLOPT_POSTFIELDS, $data_json);
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
			$response  = curl_exec($ch);
			...
			if ($response === FALSE) {
				...
				if($httpCode == 400) {
					...
					$this->auditlog("delete Success", $thingid);
            header("Location: list.php");
				}
			}
            ...
            $this->auditlog("delete Error", $errors);
        }
